...
writeFileSync(join(bundle, '使用说明.txt'), [
  ...
  '',
  '【怎么启动】',
  '  · 装了安装版：开始菜单 / 桌面上的 LHIC 图标，双击就行 —— **不会弹命令行窗口**。',
  `  · 绿色包：双击「${launcherName}」。它会带一个命令行窗口，那是日志与报错的出口，`,
  '    关掉它等于关掉应用。想要没有命令行的启动方式，就用安装版。',
  '',
  DEFAULT_PASSWORD === ''
    ? '【第一次打开】会让你设访问密码、选出图后端（都可以跳过）。'
    : `【第一次打开】会要访问密码：已经预置好了（${DEFAULT_PASSWORD}），登录页会自动填上，点「进入」即可。`,
  DEFAULT_PASSWORD === '' ? '' : '想换成自己的密码：进设置页改（改完之后预置的那个就失效了）。',
  '',
  '【东西存在哪】数据（画布、生成的图、上传的素材）都在这个文件夹的 data\\ 里：',
  '  · 想搬家/备份 → 整个文件夹拷走；',
  '  · 想放到别的盘 → 设环境变量 STUDIO_DATA_DIR 指向那个目录。',
  '  · 出问题时看 data\\desktop.log（安装版没有命令行窗口，日志都在这个文件里）。',
  '',
  '【出图需要什么】另外一份 ComfyUI（本机显卡）或火山方舟的 Key —— 这个包里不含模型，',
  '也不含 ComfyUI（它是 GPL 软件，不随本产品分发）。在向导或设置页里填地址即可。',
  '',
  '【卸载】就是删掉这个文件夹（安装版用「卸载 LHIC」）。删之前先把 data\\ 里想要的东西拷出来。',
  '',
  '端口默认 8080，被占用时会自动往后找一个。',
  '',
