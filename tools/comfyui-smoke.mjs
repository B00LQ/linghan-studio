 ...
 * 用法: node tools/comfyui-smoke.mjs [templateJson] [prompt]
 */
import { readFile } from 'node:fs/promises'
import { dirname, join, resolve } from 'node:path'
import { fileURLToPath } from 'node:url'

// 相对脚本自身定位模板（脚本在 tools/ 下，仓库根在上一层），换台机器/换个克隆目录都能跑。
const repo = resolve(dirname(fileURLToPath(import.meta.url)), '..')
const TEMPLATE = process.argv[2] ?? join(repo, 'apps/server/src/comfyui/z-image-turbo.json')
