...
| 节点：**文本 / 图片 / 视频 / 裁切 / 拼接 / 音频** | ✅ 图片与视频节点自带提示词、自己出片；裁切与拼接**不过扩散模型**（ComfyUI 核心的 Video Slice / ConcatenateVideo），秒级完成；音频节点出人声 |
...
| 端口与类型校验 | ✅ 文本→提示词；图片→视频的「首帧/尾帧」或图片的「参考图」；视频→裁切/拼接；连错类型画不出线 |
| 底部居中浮动条 | ✅ 缩放（−/百分比/＋/适应）+ 整理布局，居中不占画布空间 |
| 资产页签 | ✅ 侧栏「画布 / 资产」，缩略图 + 体积；`GET /api/assets` |
| 缩略图 | ✅ `GET /api/assets/<id>/thumb?w=320`：服务端自己解 PNG 并做块平均缩放（1.4 MB → ~120 KB），磁盘缓存；JPEG/视频暂无（回 404，前端退回原图） |
| 素材上传 | ✅ `POST /api/assets`（正文即文件），内容寻址自动去重；上传的素材与生成结果同一种节点 |
| 素材文件夹 | ✅ 素材窗的「素材 / 管理」两个页签：**素材**里按文件夹筛（全部 / 未分组 / 自建），**管理**是一张表（体积、时间、逐个归类）。`/api/asset-folders`；文件夹是**标签不是容器** —— 删掉它里面的素材一个都不少 |
| 绿色包 / 安装程序 | ✅ `node packaging/build-desktop.mjs` 一条命令：绿色包（自带 Node，双击即用）+ 更新包 + `update.json`；装了 Inno Setup 还能出 `setup.exe`（按用户安装、卸载不删 `data\`）。见 `packaging/README.md` |
| 首启向导 | ✅ 没设密码时先走三步（数据目录 → 访问密码 → 出图后端），每步可跳、跳过的代价写在旁边 |
| 自助更新 | ✅ 设置页 → 软件更新：读更新源 → 下载 → 校验 sha256 → 解到 `versions/<版本>` → 换指针 → 重启生效。**只有绿色包能自助更新**，Docker/源码运行明确拒绝 |
...
| Agent 双入口 | ✅ **外部 Agent 可用 HTTP 独立完成创作**（10 个工具，生成走作业、无人值守）；人在场时画布自动同步 |
| 生成历史（原 Shot / Take） | ✅ 每个图片节点持有自己的版本历史：提示词/种子/耗时/失败留痕，点开节点即可见 |
| 本地算力后端（ComfyUI / Z-Image Turbo） | ✅ 已端到端实测：本机 4070 出图，热态 **~6 s / 张**（1024×1024）；接参考图即图生图 |
| **设置页** | ✅ 地址与密钥都能在界面上改、能「测一下」，**改完立刻生效**（不用重启）；带「接上自己的后端」的分步引导 |
| 文本节点：**接 LLM 写故事/场景** | ✅ 节点里写下指令、按 ↑ 出文本；任意 OpenAI 兼容接口（ChatGPT / DeepSeek / Moonshot / 方舟）；没配 key 时是自称占位的占位文本 |
| 音频节点：**把文字念成人声** | ✅ 旁白/对白；任意 OpenAI 兼容的 `/audio/speech`；没配 key 时出一段**真能播的占位音**（合成 WAV，不是空文件） |
| **视频剪接（裁切 / 拼接）** | ✅ 接上片子就给一段：裁切按秒、拼接按左右顺序，**秒级**完成（用 ComfyUI 核心的 Video Slice / ConcatenateVideo，不过扩散模型） |
| Docker 一键部署 | ✅ `docker compose up -d --build` 已实测：容器内登录、建项目、出图、素材落盘到宿主机卷 |

## 图片后端（`STUDIO_IMAGE_DRIVER`）

> **这三个后端的所有配置都能在界面上改**：左侧「设置」页把它们摆出来、能改、能「测一下」，
> 改完**立刻生效**（不用重启容器）。下面这些环境变量仍然有效 —— 设置页的值压得住它们，
> 所以两种用法都成立：纯环境变量部署的人什么都不用动。
...
文本节点要能写，就得有个模型。**只支持一种接口**：OpenAI 兼容的 `/chat/completions` ——
ChatGPT、DeepSeek、Moonshot、火山方舟都是这一套，所以不必为每家写一个驱动。

```bash
# studio/.env
STUDIO_TEXT_API_KEY=sk-…            # 不给就是占位文本（会自称占位）
STUDIO_TEXT_BASE_URL=https://api.deepseek.com/v1   # 默认 https://api.openai.com/v1
STUDIO_TEXT_MODEL=deepseek-chat     # 默认 gpt-4o-mini
```

没配 key 时文本节点**照样能按**，只是写进节点的是「［占位文本 · 未配置文本模型］…」——
它的用处是让这条链在没有 key 的机器上也能被走通、被测，而不是冒充模型。
`GET /api/text-backend` 会如实回答配没配、用的哪个模型。

## 音频后端（音频节点用）

视频自带声音，但**独立的一段人声**（旁白、对白）是另一件事。同样只支持一种接口：
OpenAI 兼容的 `/audio/speech`。

```bash
# studio/.env
STUDIO_AUDIO_API_KEY=sk-…           # 不给就是占位音
STUDIO_AUDIO_BASE_URL=https://api.openai.com/v1   # 默认同上
STUDIO_AUDIO_MODEL=gpt-4o-mini-tts  # 默认 gpt-4o-mini-tts
STUDIO_AUDIO_VOICE=alloy            # 默认 alloy
STUDIO_AUDIO_FORMAT=mp3             # 默认 mp3（mp3/opus/aac/flac/wav/pcm）
```

没配 key 时出的是**一段真能播的 WAV**（服务端自己合成的短音，带淡入淡出）——
为什么不是空文件：空文件在播放器里什么都不发生，而「播放器坏了」和「没配模型」
是两回事。音频走的是和渲染一样的路（存成 `audio` 素材、记 take、写回节点），
所以版本条与「复现这一版」对它一样有效。
...
**这些数字只在画布上看得到本机自己的那一份**：下拉里每套工作流后面接着「几步 · 本机约多久」，
数字来自这台机器自己跑过的 take（最近 20 条的中位数），没跑过就只显示步数 ——
上面那行手测数字换台机器就是假话，所以不写进界面。悬停还会说清取舍：
8 步是原版配方（细节最足最慢），4 步是蒸馏加速版（更快、细节略差、片越长省得越多）。
...
**图生视频**：把一张图接到视频节点的「首帧」（或「尾帧」）入边就行，不接就是文生视频。
要手动验一遍这条链路：

```bash
VIDEO_WORKFLOW=minimax-h3-video VIDEO_SIZE=768x448 VIDEO_DURATION=0.75 \
VIDEO_FIRST_FRAME=<素材 id> node video-e2e-test.mjs http://127.0.0.1:8080 <密码>
```

（它会断言 take 里记下了首帧；「生成片的第一帧长得像那张原图」这件事只能自己看一眼。）
...
apps/server             单进程服务端
  ...
  ├── agent.ts          Agent 工具面：10 个工具 + JSON Schema，含自动建镜头与记账
  ├── store.ts          SQLite：项目 / 画布 / 镜头 / take / 素材
  └── png.ts            零依赖 PNG 编 + 解 + 缩放（占位图与缩略图都用它）
```
...
**`canvas_generate` 不等出完**：它提交作业、有上限地等一会儿（图片默认 20 秒、视频 0），
等不到就返回 `jobId`，用 `job_status` 接着查、`job_cancel` 中止。这样视频（十几分钟）
也不会把调用方的 HTTP 请求拖到超时——而「调用方失败」和「活干完了」曾经会同时为真。
注意冷启动出第一张图要 70–90 秒（装载权重），所以第一次调用通常需要轮询一次。
...
| **P1** 有自己的东西 | 本地 ComfyUI 驱动 ✅；Shot/Take 进画布 UI ✅；视频节点 ✅（MiniMax H3，含音频，支持首帧/尾帧图生视频）；Agent 工具面接通 ✅ |
| **P2** 专业化 | 导演台（3D 摆位）、逐帧拉片、片段重拍、时间线与智能剪辑；视频剪辑/拼接 |
| **P3** 可安装 | 桌面安装包 / 单文件可执行 |

## 已知限制

- **未做多租户**：一个部署一份密码，项目对所有登录者可见。要做团队/计费需要加账号体系。
- **版本可以复现，也可以重试**：版本条下面「↻ 复现这一版」用**同样参数、同样种子**
  再跑一遍（同一条版本线）；**点失败的那一格就是重试它**。要「同参数换一个种子」现在
  还没有入口（那是另一件事：复现 vs 再抽一次）。
...
- **音频节点要一个语音模型才能出人声**：不配就是占位音（能播，但不是人声）。
- **视频只有两档画幅**（1344×768 / 768×448）与三档时长实测过，菜单里只放这两档。
- **图生视频只有视频节点有**：视频节点的「首帧 / 尾帧」入边接上一张图就是图生视频
  （不接就是文生视频）；接的图由**服务端**从画布入边解析、读字节、上传到 ComfyUI 的 input
  目录，所以画布、Agent、HTTP 三条路都支持。用 `/v1/images/generations` 时没有画布节点可解析，
  要显式给 `first_frame`（素材 id 或 `/api/assets/<id>`）。
- **图片节点接一张参考图就是图生图**：接上之后默认改用图生图那套工作流（`denoise 0.6`，
  形状跟着参考图、内容跟提示词走）；不接就是文生图。选了图生图却没接参考图时**会被拒绝**
  并说明缺什么——而不是跑出一张和参考图毫无关系的图。`/v1` 那条路用 `reference` 指定。
- **画布是整份覆盖写**：两人同时编辑同一项目会互相覆盖（Agent 与人的并发也是同一问题）。
- **粘贴未实现**：右键菜单里因此没有「粘贴」条目（不放点了没反应的项）。
- **文本节点要一个 LLM 才能写**：不配就是占位文本（自称占位，不冒充模型）。
- **输出端口的下游已经齐了**：文本 → 提示词、图片 → 视频的首帧/尾帧、图片 → 图片的参考图、
  视频 → 裁切/拼接。还没有的是「视频 → 视频」的其它处理（转场、变速、字幕）。
- **框住恰好一个节点 = 点它一下**（会打开提示词窗口）；框住多个由多选工具条接管。
- **编辑会记成版本**：旋转/裁剪都存成一个新版本，原图永远点得回去；**连续同一种快捷编辑
  只留一版**（连点四次「右转 90°」在版本条上仍是一格），换了改法或切过版本才会再记一版。
  被替换下来的中间素材会留在素材库里（替换那一刻画布文档还指着它，服务端不敢删）。
- **音频节点要一个语音模型才能出人声**：不配就是占位音（能播，但不是人声）。
